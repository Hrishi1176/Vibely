...
import { Sparkles, PlusCircle, User as UserIcon, Zap, LogOut, HelpCircle, Download } from 'lucide-react';
      ...
      <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between">
        ...
        >
          <div className="relative">
            ...
            <img
              src="/logo.svg"
              alt="Vibely"
              className="relative w-9 h-9 transition-transform group-hover:scale-110 group-hover:rotate-[-6deg] duration-300"
            />
          </div>
          <span className="font-['Outfit'] text-2xl font-extrabold tracking-tight gradient-text">
            Vibely
          </span>
        ...
        <div className="flex items-center space-x-2 sm:space-x-3">
          ...
          <button
            ...
            className="p-2 rounded-xl bg-[var(--bg-secondary)] hover:bg-[var(--scrollbar-thumb)] border border-[var(--border-glass)] text-[var(--accent-primary)] text-xs font-semibold flex items-center space-x-1 transition-all"
            title="Why Vibely?"
          >
          ...
          {user ? (
            <>
              <button
                ...
                className="btn-gradient btn-glow-pulse px-3.5 py-2 rounded-xl text-white font-semibold text-sm flex items-center space-x-2"
              >
                <PlusCircle className="w-4 h-4" />
                <span className="hidden sm:inline">New Vibe</span>
              </button>

              <div className="flex items-center space-x-2 border-l border-[var(--border-glass)] pl-3">
                <button
                  onClick={() => setActiveTab('profile')}
                  className="relative flex items-center p-0.5 rounded-full transition-all hover:scale-110"
                >
                  <div className="absolute inset-0 rounded-full animated-border p-[1.5px]" />
                  <img
                    src={user.avatar_url || `https://api.dicebear.com/7.x/bottts/svg?seed=${user.username}`}
                    alt={user.username}
                    className="relative w-8 h-8 rounded-full object-cover border-2 border-[var(--bg-primary)]"
                  />
                </button>
                <button
                  onClick={onLogout}
            ...
            <button
              onClick={onOpenAuth}
              className="btn-gradient btn-glow-pulse px-4 py-2 rounded-xl text-white font-semibold text-sm flex items-center space-x-2"
            >
